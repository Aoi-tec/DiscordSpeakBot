...
def configure_first_join(store, guild_id, text_channel_id, voice_channel_id):
    ...
    store.update("guilds", store.get("guilds").revision, mutate)
    log.info(
...
class SpeakBot(discord.Client):
    ...
    def install_commands(self):
        ...
        async def run_action(interaction, name):
            ...
            if name == "join" and guild_id not in self.runtime.store.get("guilds").guilds:
                voice = getattr(interaction.user, "voice", None)
                channel = voice.channel if voice else None
                if not isinstance(channel, discord.VoiceChannel):
                    await interaction.followup.send(
                        "初回設定には、先に通常のVCへ参加してから/tts joinを実行してください。",
                        ephemeral=True,
                    )
                    return
            ...
            try:
                await self.action(guild_id, name)
            except ValueError as exc:
                await interaction.followup.send(str(exc), ephemeral=True)
                return
            except discord.Forbidden:
                await interaction.followup.send(
                    "Botに対象VCの表示・接続・発話権限がありません。", ephemeral=True
                )
                return
            except (discord.DiscordException, asyncio.TimeoutError) as exc:
                log.warning("voice_action_failed guild_id=%s type=%s", guild_id, type(exc).__name__)
                await interaction.followup.send(
                    "VC接続に失敗しました。Botの権限とVoice接続状態を確認してください。",
                    ephemeral=True,
                )
                return
